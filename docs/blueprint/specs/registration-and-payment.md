...
2. Client gửi một API Request (`POST /v1/tickets/register`) đính kèm `X-Idempotency-Key` (dạng UUID) trên Header để định danh duy nhất cho thao tác đăng ký này.
...
- **Lỗi mạng khi thanh toán / Retry**: Nếu request đăng ký hoặc thanh toán bị timeout hoặc rớt mạng, Client có thể gọi lại API kèm theo cùng một `X-Idempotency-Key` ban đầu. Backend đối chiếu key này (lưu trong Redis/DB) để nhận diện giao dịch trùng lặp, bỏ qua thao tác trừ tiền/giữ chỗ lần 2 và chỉ trả về trạng thái hiện tại.
...
- **Idempotency**: Cả API đăng ký và API thanh toán phải mang tính Idempotent. Một `X-Idempotency-Key` cần có thời gian hiệu lực cache ít nhất 24 giờ.
...
- Gửi 2 request đăng ký/thanh toán giống hệt nhau (cùng `X-Idempotency-Key`) liên tiếp không được phép sinh ra 2 vé hay trừ tiền 2 lần.
