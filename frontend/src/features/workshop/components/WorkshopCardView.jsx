    ...
    <div className="space-y-1.5">
      <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">
        <span className="font-mono">
          {available === 0 ? "STATUS: FULL" : `${available} SLOTS REMAINING`}
        </span>
    ...
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {workshops.map((w) => (
        ...
        >
          ...
          {/* Status accent bar */}
          <div
            className={`h-1 w-full relative z-10 ${STATUS_ACCENT[w.status] ?? "bg-gray-300"}`}
          />
          ...
          <div className="relative z-10 flex flex-col flex-1 p-6 gap-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div
                  className={`w-1.5 h-1.5 rounded-full ${w.status === "published" ? "bg-cyan-500 animate-pulse shadow-[0_0_8px_rgba(34,211,238,0.8)]" : "bg-gray-400"}`}
                ></div>
                <StatusBadge
                  status={w.status}
                  className="bg-transparent p-0 text-[10px] font-black uppercase tracking-widest text-gray-500"
                />
              </div>
              <span
            ...
            <div className="grid grid-cols-1 gap-3 py-4 border-y border-gray-50 dark:border-gray-800/50">
              <div className="flex items-center gap-3 text-xs font-bold text-gray-700 dark:text-gray-300">
                ...
                <span className="font-mono">
                  {formatDate(w.event_day)} · {formatTime(w.start_time)}
                </span>
              </div>
              <div className="flex items-center gap-3 text-xs font-bold text-gray-700 dark:text-gray-300">
                <svg
