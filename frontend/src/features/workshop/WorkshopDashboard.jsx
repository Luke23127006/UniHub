    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-[140px] shrink-0 h-[38px] text-sm rounded-lg border-unihub-border dark:border-gray-600 bg-unihub-card dark:bg-gray-800 text-unihub-text dark:text-gray-100 focus:ring-unihub-primary/40 focus:outline-none">
            <SelectValue placeholder="Status" />
          </SelectTrigger>
          <SelectContent className="rounded-lg border-unihub-border dark:border-gray-600 bg-unihub-card dark:bg-gray-800 shadow-lg">
            {STATUS_FILTERS.map((f) => (
              <SelectItem key={f.value} value={f.value} className="cursor-pointer text-sm text-unihub-text dark:text-gray-100">
                {f.label}
              </SelectItem>
            ))}
          </SelectContent>
