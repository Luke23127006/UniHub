...
export default function WorkshopDetailPage() {
  ...
  const occupancyPercentage = Math.min(
    100,
    Math.max(0, ((capacity - seatsAvailable) / capacity) * 100),
  );
  ...
  return (
    <div className="relative min-h-[calc(100vh-8rem)] py-8 bg-white dark:bg-[#050505]">
      ...
      <div
        className="absolute inset-0 z-0 pointer-events-none opacity-[0.03] dark:opacity-[0.06]"
        style={{
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10 space-y-10">
        ...
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-cyan-500 transition-colors group"
        >
          <svg
          ...
          <div className="relative flex flex-col md:flex-row gap-10 items-start justify-between">
            <div className="flex-1 space-y-6">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 italic">
                  Research & Workshop
                </span>
              </div>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {[
            {
              label: "Time",
              value: formatDateTime(workshop.start_time),
              icon: <ClockIcon />,
              sub: "Verification Window",
              bg: "bg-cyan-500/5 dark:bg-cyan-500/10",
            },
            {
              label: "Location",
              value: workshop.room?.room_code,
              icon: <MapIcon />,
              sub: workshop.room?.building,
              bg: "bg-purple-500/5 dark:bg-purple-500/10",
            },
            {
              label: "Capacity",
              value: `${seatsAvailable} / ${capacity}`,
              icon: <UsersIcon />,
              sub: "Active Registry",
            ...
            {
              label: "Access Fee",
              value: formatPrice(workshop.price, workshop.currency),
              icon: <TagIcon />,
              sub: "Authenticated",
            ...
            <div
              key={i}
              className="bg-white/40 dark:bg-gray-900/40 border border-gray-100 dark:border-gray-800 rounded-3xl p-6 hover:border-gray-200 dark:hover:border-gray-700 transition-all group"
            >
              <div className="flex items-center gap-4 mb-4">
                <div
                  className={`p-2.5 rounded-xl border border-gray-100 dark:border-gray-800 group-hover:scale-110 transition-transform ${item.bg}`}
                >
                  {item.icon}
                </div>
                <div>
              ...
              {item.sub && (
                <p className="mt-3 text-[9px] font-mono text-gray-400 uppercase tracking-widest">
                  {item.sub}
                </p>
              )}
            </div>
          ...
          <section>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {workshop.speakers.map((speaker, index) => (
                <div
                  key={index}
                  className="flex items-center gap-4 p-5 rounded-2xl border border-gray-100 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors"
                >
