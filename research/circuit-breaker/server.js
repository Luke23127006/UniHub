  ...
  if (isPaymentGatewayDown) {
    return res
      ...
      .json({
        ...
        message: "Cổng thanh toán đang bảo trì",
      });
  }
  return res
    .status(200)
    .json({ status: "SUCCESS", transaction_id: "TXN12345" });
});
...
const callPaymentAPI = async () => {
  const response = await axios.post(
    `http://localhost:${PORT}/api/mock-payment`,
    {},
    { timeout: 3000 },
  );
...
app.post("/checkout", async (req, res) => {
  try {
    ...
    const statusCode =
      error.message && error.message.includes("CircuitBreaker")
        ? 503
