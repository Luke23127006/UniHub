...
async function processRegistrationMessage(msg, channel) {
  ...
  if (!lockToken) {
    console.log(
      `Could not acquire lock for workshop ${workshopId}. Requeueing after ${LOCK_RETRY_DELAY_MS}ms.`,
    );
  ...
  try {
    const result = await WorkshopService.processRegistration(
      userId,
      workshopId,
    );
...
async function startRegistrationWorker() {
  try {
    ...
    console.log(
      `[*] Waiting for messages in ${REGISTRATION_QUEUE}. To exit press CTRL+C`,
    );
